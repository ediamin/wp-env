...
const stop = require( '@wordpress/env/lib/commands/stop' );
const initConfig = require( '../init-config' );
const downloadSources = require( '@wordpress/env/lib/download-sources' );
const {
	checkDatabaseConnection,
	configureWordPress,
} = require( '@wordpress/env/lib/wordpress' );
const { didCacheChange, setCache } = require( '@wordpress/env/lib/cache' );
...
module.exports = async function start( { spinner, debug, update, xdebug } ) {
	...
	if ( shouldConfigureWp ) {
		...
		// also update WordPress. Since we store wordpress files
	...
	await dockerCompose.upOne( 'wordpress', {
		...dockerComposeConfig,
		commandOptions: shouldConfigureWp
			? [ '--build', '--force-recreate' ]
			: [],
