...
const { readFile, appendFile, writeFile, mkdir } = require( 'fs' ).promises;
...
module.exports = async function initConfig( {
	...
	if ( writeChanges ) {
        ...
        const dockerfileExtra = path.resolve( 'Dockerfile' );
        if ( existsSync( dockerfileExtra ) ) {
            await appendFile(
                path.resolve( config.workDirectoryPath, 'Dockerfile' ),
                await addDockerfileExtraConfig(dockerfileExtra)
            );
        }
...
async function addDockerfileExtraConfig(dockerfilePath) {
    const config = await readFile( dockerfilePath, 'utf8' );

    return `# Config from ${dockerfilePath}
${config}
`;
}
